class Member < ApplicationRecord
  ...
  def self.record_action(payload, route)
    ...
    begin
      ...
      if member = Member.upsert_member(payload[:cons_hash].merge(updated_at: payload[:create_dt]), "action:#{payload[:action_name]}")
        ...
        if payload[:action_name] != action.name
          action.update_attribute(:name, payload[:action_name])
        end
        ...
        if !action.campaign && action.technical_type == 'cby_petition'
          if campaign = Campaign.find_by(controlshift_campaign_id: action.external_id, campaign_type: 'controlshift')
            action.update_attribute(:campaign_id, campaign.id)
          end
        end
        ...
        if payload[:consents].present?
          payload[:consents].each do |consent_hash|
            ...
            consent_text = ConsentText.find_by_public_id!(consent_hash[:public_id])
            ...
            member_action.member_action_consents.build(
              ...
              parent_member_action_consent: nil, # TODO: Something like `member.member_current_consents.find_by(consent_public_id: consent_text.public_id).member_action_consent` but only if it's a 'no_change'...
              created_at: payload[:create_dt],
        ...
        if !Settings.gdpr.disable_auto_subscribe && (payload[:opt_in].nil? || payload[:opt_in]) && !member.subscribed?
          email_subscription = member.member_subscriptions.find_by(subscription_id: Subscription::EMAIL_SUBSCRIPTION)
        ...
        if member_action.new_record? and member_action.valid?
          ApplicationRecord.transaction do
            # create member action
            member_action.save!
            ...
            if payload[:source]
              if source = Source.find_or_create_by(source: payload[:source][:source],
                                                   medium: payload[:source][:medium],
                                                   campaign: payload[:source][:campaign])
                member_action.update_attribute(:source_id, source.id)
              else
                Rails.logger.info "Source failed to be saved for member action #{member_action.id}. Source payload: #{payload[:source].inspect}"
              end
            end
            ...
            if payload[:survey_responses]
              payload[:survey_responses].each do |sr|
                ...
                unless question = Question.find_by(action_key: action_key)
                  Question.create!(action_key: action_key,
                                   question_type: sr[:question][:qtype])
                end
